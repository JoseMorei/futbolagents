...
class EvaluationDatasetGenerator:
    ...
    def __call__(self, players: list[PlayerExtract]) -> EvaluationDataset:
        dataset_samples = []
        extraction_generator = get_extraction_generator(players)
        for player, docs in extraction_generator:
            chunks = self.__splitter.split_documents(docs)
            for chunk in chunks[:4]:
                try:
                    dataset_sample: EvaluationDatasetSample = self.__chain.invoke(
                        {"philosopher": player, "document": chunk.page_content}
                    )
                except Exception as e:
                    logger.error(f"Error generating dataset sample: {e}")
                    continue

                dataset_sample.philosopher_id = player.id
