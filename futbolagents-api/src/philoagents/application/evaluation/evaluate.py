...
async def evaluation_task(x: dict) -> dict:
    """Calls agentic app logic to evaluate player responses.
    ...
    Args:
        x: Dictionary containing evaluation data with the following keys:
            ...
            philosopher_id: ID of the player to use
    ...
    Returns:
        dict: Dictionary with evaluation results containing:
            ...
            output: Generated response from player
            expected_output: Expected answer for comparison
    """

    player_factory = PlayerFactory()
    player = player_factory.get_player(x["philosopher_id"])
...
def get_used_prompts() -> list[opik.Prompt]:
    ...
    prompts = [
        client.get_prompt(name="player_character_card"),
