...
@click.option(
    ...
    help="Path to the metadata file containing player extracts",
)
@click.option(
    "--temperature",
    type=float,
...
def main(metadata_file: Path, temperature: float, max_samples: int) -> None:
    """
    Generate an evaluation dataset from player extracts.

    Args:
        metadata_file: Path to the metadata file containing player extracts
    ...
    evaluation_dataset_generator(players)
